...
const SettingsScreen = () => {
  ...
  const General: { title: string, route: string, icon: keyof typeof Ionicons.glyphMap }[] = [
    ...
    { title: 'Theme', route: '/settings/theme', icon: 'color-palette-outline' },
  ];
  ...
  const renderSection = (title: string, list: typeof General) => (
  ...
  return (
    <SafeAreaView style={styles.container}>
      ...
      <ScrollView contentContainerStyle={styles.scrollViewContent} showsVerticalScrollIndicator={false}>
        ...
        {integrationList.length > 0 && renderSection('INTEGRATIONS', integrationList)}
        {resourcesList.length > 0 && renderSection('RESOURCES', resourcesList)}
        {showContact && renderSection('CONTACT', contactList)}
...
const styles = StyleSheet.create({
  container: { flex: 1 },
  scrollViewContent: { paddingBottom: 40, maxWidth: 780, margin: 'auto', width: '100%' },
  headerContainer: { paddingHorizontal: 20, paddingTop: 8, paddingBottom: 24 },
  headerTitle: { fontSize: 34, fontWeight: '700', letterSpacing: 0.3 },
  section: { marginBottom: 32 },
  sectionHeader: { fontSize: 13, fontWeight: '500', textTransform: 'uppercase', marginBottom: 10, marginLeft: 20, marginRight: 20, letterSpacing: 0.5 },
  settingsGroup: { marginHorizontal: 16, overflow: 'hidden', borderRadius: 12 },
  settingItem: { flexDirection: 'row', paddingVertical: 14, paddingHorizontal: 16, justifyContent: 'space-between', alignItems: 'center', minHeight: 52, position: 'relative' },
  ...
  settingText: { fontSize: 17, fontWeight: '400', color: '#FFFFFF', letterSpacing: -0.2 },
  separator: { position: 'absolute', bottom: 0, left: 60, right: 16, height: StyleSheet.hairlineWidth, backgroundColor: '#2a2a2a' },
  versionContainer: { paddingVertical: 20, flexDirection: 'row', justifyContent: 'center', alignItems: 'center', marginTop: 8 },
  versionText: { fontSize: 13, color: '#6E6E73', fontWeight: '500', letterSpacing: 0.2 },
});
